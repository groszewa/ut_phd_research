...
module dsc_mul(a, b, c, d, z, clk, rst, en, ov);
  ...
  wire ctr_ov_a, ctr_ov_b, ctr_ov_c, dummy_ov, n_0, n_1, sn_out_a,
       sn_out_b;
  wire sn_out_c, sn_out_d, sn_out_mul;
  ...
  prg_8b_3 prg_d(.clk (ctr_ov_c), .rst (rst), .en (en), .bin_in (d),
       .sn_out (sn_out_d), .ctr_overflow (ov));
  counter_WIDTH32 stoch2bin_out(.out (z), .clk (clk), .en (sn_out_mul),
       .rst (rst), .overflow (dummy_ov));
  AND2X1 g23(.A (n_0), .B (n_1), .Y (sn_out_mul));
  AND2X1 g24(.A (sn_out_a), .B (sn_out_b), .Y (n_1));
  AND2X1 g25(.A (sn_out_d), .B (sn_out_c), .Y (n_0));
endmodule
